...
template <class cohttp_steam> class cohttpsvr_base {
public:
...
private:
  ...
  std::shared_ptr<http_dispachter> session_dispatcher_;
...
public:
  cohttpsvr_base(const std::string &host, const unsigned short port)
    ...
    session_dispatcher_ = std::make_shared<http_dispachter>();
  }
...
protected:
  ...
  virtual void go_session(std::shared_ptr<tcp::socket> socket,
                          std::shared_ptr<http_dispachter> dispatcher) = 0;
...
protected:
  static void do_dispatcher(const http::request<http::string_body> &req,
    ...
    auto session_call = dispatcher->find(std::string(req.target()));
    if (session_call == dispatcher->end()) {
      session_call = dispatcher->find("404");
      BOOST_ASSERT(session_call != dispatcher->end());
    }
...
public:
  void register_404(const http_session &session) {
    (*session_dispatcher_)["404"] = session;
  }

  void register_uri(const std::string &uri, const http_session &session) {
    (*session_dispatcher_)[uri] = session;
  }
  ...
  void accept() {
    ...
    dco_go[this] {
      for (;;) {
        ...
        go_session(socket, session_dispatcher_);
      }
    };
  }
