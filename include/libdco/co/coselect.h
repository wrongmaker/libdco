...
public:
  ...
  public:
    cocaseopt(opt_type opt, cochannel_base *base, coctx *ctx,
        ...
        : code_(0), opt_(opt), base_(base),
          node_(ctx, payload, [this] { code_ = 1; }), func_(func) {}
    ~cocaseopt() {}
  };
...
public:
  template <class T>
  coselect &wait_case(cochannel<T> *chan, T &t,
  ...
  template <class T>
  coselect &post_case(cochannel<T> *chan, const T &t,
                      const std::function<void(int)> &func) {
